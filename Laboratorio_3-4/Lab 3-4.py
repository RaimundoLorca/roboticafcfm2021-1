...
#Cargar imagen
img = cv2.imread('Imagenes//batch_1.png') #'Imagenes//batch_1.png' 'Imagenes//part_ref.png'
...
print('\nNúmero de lineas detectadas:' , len(rectas))

#%% LAB 4 - Robótica 2021-1
...
for recta in rectas:
  ...
  #Se asume que todas las piezas están alineadas horizontalmente!
  if abs(theta*180/np.pi - 30.0) < 5:
...
print('\nÁngulo de corte identificado: {:f} grados.'.format(cut_angle))
...
#Cambiar a imagen de referencia!!!!!!!!!!

#Calculo medidas de referencia:

a1 = abs(perf_izq[0][0] - perf_der[0])
a2 = abs(perf_izq[1][0] - perf_der[0])

a = (a1+a2)/2
print('\na = ', a)

b1 = abs(perf_izq[0][1] - perf_der[1])
b2 = abs(perf_izq[1][1] - perf_der[1])

b = (b1+b2)/2
print('\nb = ', b)

#Cuanto pixeles hay por unidad de medida (mm/pixel)
d_p = 45/a

print('\nRelación distancia_real/pixel:', d_p)
...
def detect_faulty_parts(img):
    ...
    for i in range(1, num_parts):
      ...
      try:
        ...
        #(Recordar que la tolerancia es de 60° ± 5%)
        assert abs( cut_angle - 60 ) < 60*0.05

        assert abs( abs(perf_izq[0][0] - perf_der[0]) - 125.5 ) < 125.5*0.05
        assert abs( abs(perf_izq[1][0] - perf_der[0]) - 125.5 ) < 125.5*0.05

        assert abs( abs(perf_izq[0][1] - perf_der[1]) - 41.5 ) < 41.5*0.1
        assert abs( abs(perf_izq[1][1] - perf_der[1]) - 41.5 ) < 41.5*0.1
...
def detect_faulty_parts_2(img):
    ...
    #Dibujar keypoints logo viejo
    old_logo = np.copy(ref)
    old_logo = cv2.drawKeypoints(old_logo, kp_ref, ref, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

    plot.figure(figsize=(10, 10))
    plot.imshow(old_logo)
    plot.axis('off')    
    ...
    for i in range(1, num_parts):
      ...
      # plot.figure(figsize=(10, 10))
      # plot.imshow(part_logo, cmap='gray')
      # plot.axis('off')
      ...
      for match_1, match_2 in matches:
        if match_1.distance < 0.5*match_2.distance:
